...
def random_create(user_list_par, server_list_par):
    # 对程序运行时间进行记录
    st_tm = time.time()
    ...
    pops = [0 for i in range(0,len(server_list))]
    ...
    for user in user_wait_allocated_list:
        ...
        if len(within_servers) > 0:
            ...
            while (ser_rem_cap[0] < user_workload[0]) or (ser_rem_cap[1] < user_workload[1]) or (
                ...
                within_servers.pop(index)  # 移除不满足条件的服务器
    ...
    return pops,[user_allo_prop,server_used_prop]
